...
(function ($) {
    ...
    var name = _.map(data, _.property('name'));
    var sortedName = [];
    var friends = _.map(data, _.property('friends'));

    /*Skills*/

    var skills = _.map(data, _.property('skills'));
    var skillsUnion = _.sortBy(_.union.apply(_, skills));

    /*sort names*/
    _(data).forEach(function (value) {
        fLength.push(value.friends.length);
    });
    ...
    for (var i = 0; i < fLength.length; i++) {
        var key = name[i];
        var value = fLength[i];
        nameForSort[name[i]] = fLength[i];
    }

    var sortableNames = [];
    for (var key in nameForSort) {
        sortableNames.push([key, nameForSort[key]]);
    }
    sortableNames.sort(function (a, b) {
        return a[1] - b[1]
    })
    for (var i = 0; i < sortableNames.length; i++) {
        sortedName.push(sortableNames[i][0]);
    }
    ...
    _(friends).forEach(function (value) {
        for (var i = 0; i < value.length; i++) {
            arrFriends.push(value[i].name);
        }
    });

    friendsUnion = _.union(_, arrFriends);
    var skillsSorted = _.sortBy(skillsUnion, function (num) {
        return num.toLowerCase();
    });


    console.log("Skills: " + skillsSorted);
    console.log("Names: " + sortedName);
    console.log("Friends: " + friendsUnion);
})(jQuery);